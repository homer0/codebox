...
2. Preserve the Dockerfile coupling: it copies `src/cli`, `.node-version`, `package.json`, and `pnpm-lock.yaml` into `/home/coder/.codebox/cli`, then runs `install.sh` and links `bin.sh` as `codeboxcli`.
