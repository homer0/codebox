...
- The root `package.json`, lockfile, and `.nvmrc` are copied into `src/cli`'s image destination by `Dockerfile`; keep them compatible with the CLI dependencies.
...
- Use Node 24 (`.nvmrc`) and pnpm 11.17.0 (`package.json`).
